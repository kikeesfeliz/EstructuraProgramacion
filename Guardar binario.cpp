/*
Trabajo final Programación estructurada.

Flores Hernandez Jesus Javier
Gamez Burgueño Luis Enrique
Palacios Sanchez Luis Fernando
Uribe Rocha Juan Fernando
*/
...
main(){
	...
	do{
		...
		do {
			...
			switch(op){
				case 'a' : sumar_matrices(n);
							n = 0;
							break;
				case 'b' : restar_matrices(n);
							n = 0;
							break;
				case 'c' : multiplicar_matrices(n);
							n = 0;
							break;
				case 'd' : transponer_matriz(n);
							n = 0;
							break;
				case 'e' : guardar_archivo(n);
							n = 0;
							break;
				case 'f' : leer_archivo(n);
							n = 0;
							break;
				case 'g' : usar_determinante(n);
							n = 0;
							break;
...
void guardar_archivo(int n) {
    ...
    if (archivo != NULL) {
        // ?? Guardar primero la dimensión
        fwrite(&n, sizeof(int), 1, archivo);

        // Guardar la matriz
        size_t elementosEscritos = fwrite(A, sizeof(int), n * n, archivo);
...
void leer_archivo(int n) {
    ...
    if (archivo != NULL) {
        // ?? Leer primero la dimensión real
        fread(&n, sizeof(int), 1, archivo);

        // Leer la matriz con la dimensión obtenida
        size_t elementosLeidos = fread(A, sizeof(int), n * n, archivo);
        if (elementosLeidos < n * n) {
            printf("\nError al leer la matriz del archivo.\n");
        } else {
...
int calcular_determinante (int n, int A[MAX][MAX]){
	...
	if (a%2 == 0){
			...
			det += (-1) * A[0][a] * calcular_determinante(n-1, submatriz);	
		}
	}
	return det;		
}
